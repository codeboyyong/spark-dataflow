...
public class HBaseIOTest {
  ...
  public void testGeneric() throws Exception {
    ...
    HBaseIO.Read.Bound bound =
    		HBaseIO.Read.from("localhost:2911","test" ,null);
    
    PCollection<String[]> input = p.apply(bound);  
    
    input.apply(ParDo.of(new StringJoiner()))
  ...
  static class StringJoiner extends DoFn<String[], String> {
    @Override
    public void processElement(ProcessContext c) throws Exception {
     String[] x = 	c.element();
     StringBuilder sb = new StringBuilder("") ;
     for (int i = 0; i < x.length; i++) {
    	 sb.append(x[i]).append("_") ;//just example
	}
